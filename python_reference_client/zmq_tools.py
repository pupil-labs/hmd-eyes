...
class ZMQ_handler(logging.Handler):
    ...
    def emit(self, record):
        self.socket.send(
            "logging.{0}".format(record.levelname.lower()), record.__dict__
        )
...
class Msg_Dispatcher(Msg_Streamer):
    ...
    def notify(self, notification):
        ...
        if notification.get("remote_notify"):
            self.send("remote_notify.{}".format(notification["subject"]), notification)
        elif notification.get("delay", 0):
            self.send("delayed_notify.{}".format(notification["subject"]), notification)
        else:
            self.send("notify.{}".format(notification["subject"]), notification)
