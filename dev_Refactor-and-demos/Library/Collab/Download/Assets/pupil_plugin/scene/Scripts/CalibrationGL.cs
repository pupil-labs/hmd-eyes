...
public static class CalibrationGL{
	...
	public static void Marker(PupilSettings.Calibration.Marker _m){
		...
		GL.Begin (GL.QUADS);
		GL.TexCoord2 (0,1);
		GL.Vertex (new Vector3 (_m.position.x-((_m.size/2)), _m.position.y-_m.size/2, _m.position.y));//BL
		GL.TexCoord2 (1,1);
		GL.Vertex (new Vector3 (_m.position.x-((_m.size/2)), _m.position.y+_m.size/2, _m.position.y));//TL
		GL.TexCoord2 (1,0);
		GL.Vertex (new Vector3 (_m.position.x+((_m.size/2)), _m.position.y+_m.size/2, _m.position.y));//TR
		GL.TexCoord2 (0,0);
		GL.Vertex (new Vector3 (_m.position.x+((_m.size/2)), _m.position.y-_m.size/2, _m.position.y));//BR
		GL.End();
