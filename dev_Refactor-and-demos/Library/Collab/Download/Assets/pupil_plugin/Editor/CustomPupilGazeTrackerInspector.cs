...
//		EditorApplication.update += CheckConnection;
	...
	public static void AutoRunLayout(){
		...
		pupilSettings.connection.isAutorun = GUILayout.Toggle (pupilSettings.connection.isAutorun, "Autorun Pupil Service", "Button");

		if (!pupilSettings.connection.isAutorun && Application.isPlaying) {
...
//		}
		...
		if (PupilSettings.Instance.connection.isConnected) {
			...
			if (pupilSettings.connection.isLocal) {
				...
				GUILayout.Label ("remote " + pupilSettings.connection.IP + " ( Connected )" , pupilTracker.Styles[11]);
			}
			...
			if (pupilSettings.connection.isLocal) {
				...
				GUILayout.Label ("remote " + pupilSettings.connection.IP + " ( Not Connected )" , pupilTracker.Styles[11]);
			}
		}
...
//			pupilTracker.DebugVariables.subscribeNotify = GUILayout.Toggle (pupilTracker.DebugVariables.subscribeNotify, "Subscribe to notifications.", "Button");
	...
	private void DrawSettings(){
		...
		case 0://PUPIL APP
			...
			pupilSettings.connection.isLocal = Convert.ToBoolean (GUILayout.Toolbar (Convert.ToInt32 (pupilSettings.connection.isLocal), new string[] {
				"Remote",
				"Local"
			...
			if (EditorGUI.EndChangeCheck ()) {
				if (pupilSettings.connection.isLocal) {
					tempServerIP = pupilSettings.connection.IP;
					pupilSettings.connection.IP = "127.0.0.1";
				} else {
					pupilSettings.connection.IP = tempServerIP;
				}
			}

			////////////////////////////CONNECTION MODE////////////////////////////
			GUILayout.Space (5);
			if (pupilSettings.connection.isLocal) {//LOCAL CONNECTION MODE//
				////////////////////////////PUPIL APP PATH////////////////////////////
				GUILayout.BeginHorizontal ();
				GUILayout.Label ("path : ", pupilTracker.Styles [6], GUILayout.MinWidth (50));

				PupilTools.GetPupilSettings ().pupilServiceApp.servicePath = EditorGUILayout.TextArea (PupilTools.GetPupilSettings ().pupilServiceApp.servicePath, pupilTracker.Styles [7], GUILayout.MinWidth (100), GUILayout.Height (22));
			...
			if (pupilSettings.customGUIVariables.bools.isAdvanced){//ADVANCED SETTING
				...
				pupilSettings.connection.PORT = EditorGUILayout.IntField (pupilSettings.connection.PORT, pupilTracker.Styles[8], GUILayout.MinWidth (100), GUILayout.Height (22));
				//
			...
			if (!pupilSettings.connection.isLocal) {//---------REMOTE CONNECTION MODE---------//
...
//				pupilTracker.Settings.a = EditorGUILayout.TextArea (pupilTracker.Settings.a, pupilTracker.Styles[8], GUILayout.MinWidth (50), GUILayout.Height (22));
				pupilSettings.connection.IP = EditorGUILayout.TextArea (pupilSettings.connection.IP, pupilTracker.Styles[8], GUILayout.MinWidth (50), GUILayout.Height (22));
		...
		case 1://CALIBRATION
			...
			if (PupilSettings.Instance.dataProcess.state == PupilSettings.EStatus.Calibration || !isEyeProcessConnected || (int)pupilSettings.calibration.currentCalibrationMode != 1) {
			} else {
