...
//	[HideInInspector]
	...
	private Queue<Task> TaskQueueMainThread = new Queue<Task> ();
	private object _queueLock = new object ();

	public void ScheduleTask(Task newTask){
		lock (_queueLock) {
			if (TaskQueueMainThread.Count < 150)
				TaskQueueMainThread.Enqueue (newTask);
		}
	}

	private Queue<Task> TaskQueueNetMQThread = new Queue<Task> ();
	private object _queueLockNetMQ = new object ();

	public void ScheduleTaskNetMQ(Task newTask){
		lock (_queueLockNetMQ) {
			if (TaskQueueNetMQThread.Count < 150)
				TaskQueueNetMQThread.Enqueue (newTask);
		}
	}
	...
	void Update(){
		...
		lock (_queueLock) {
			if (TaskQueueMainThread.Count > 0)
				TaskQueueMainThread.Dequeue () ();
		}
...
//			print ("confidence assigned on pupil 1 notification : " + Pupil.values.Confidences [1]);
		...
		case "gaze":
			...
			if (_pupilDataDict.GetValueAsFloat ("confidence") > 0.4f)
				OnGazePacket ();
			break;
			...
			while ( _isDone == false)
			{
				...
				if (_isconnected)
				{
					...
					try
					{
						...
						lock (_queueLockNetMQ) {
							if (TaskQueueNetMQThread.Count > 0)
								TaskQueueNetMQThread.Dequeue () ();
						}
...
//					print("Failed to receive a message.");
	...
	public void StartPupilServiceRecording (string path){
		var _p = path.Substring (2);
		ScheduleTaskNetMQ (new Task (delegate {
//			print("from the netmq thread " + _p);
			_sendRequestMessage (new Dictionary<string,object> {{"subject","recording.should_start"},{"session_name",_p}});
		}));
	}
	...
	void OnGazePacket(){
		...
		if (m_status == EStatus.ProcessingGaze) {
			...
			//_m.shape.width =
			float alphaRatio = Mathf.InverseLerp (DefaultCalibrationCount, 0f, _currCalibSamples);//*_m.baseSize;//size hardcoded, change this
			_m.color = new Color(1f,1f,1f, alphaRatio);
