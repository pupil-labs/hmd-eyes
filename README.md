...
1. Start Pupil Service via the `pupil_service.exe` in the Pupil Service App folder. A window like this will appear when service is running. Service does pupil detection from eye images. ![Pupil Service with Eye Window](.docs/EyeWindow.png)
...
4. After a successful calibration, this scene will be loaded 
	...
	![Market Scene Demo](./docs/2DMarketScene.png)
...
1. Select the `Plugin Manager` in `Pupil Capture` and start the `HoloLens Relay` plugin ![Pupil Capture with HoloLens Relay](./docs/PupilCaptureWithHoloLensRelay.png)
...
1. Follow the on device instructions to open the menu (double air tap) and select `Connect to Pupil` by looking straight at the button and confirming with a single air tap. ![HoloLens Menu](./docs/HoloLensMenu.png)
...
1. After a successful calibration, you should see a rotating 3D model  and a visualization of your gaze ![Shark Demo with Gaze Visualization](./docs/2DDemoHoloLens.png)
